...
def does_alien_touch_wall(alien: Alien, walls: List[Tuple[int]]):
    ...
    """
    for wall in walls:
        swall = ((wall[0], wall[1]), (wall[2], wall[3]))
        if alien.get_shape() == "Ball":
            loc, r = alien.get_centroid(), alien.get_width()
            if point_segment_distance(loc, swall) <= r:
                return True
        elif alien.get_shape() == "Horizontal" or alien.get_shape() == "Vertical":
            x, y = alien.get_centroid()
            w, h = alien.get_width(), alien.get_length()/2

            if alien.get_shape() == "Horizontal":
                vertices=((x-h,y-w),(x-h,y+w),(x+h,y+w),(x+h,y-w)) 
            else:
                vertices=((x-w,y-h),(x-w,y+h),(x+w,y+h),(x+w,y-h))

            for i in range(4):
                if do_segments_intersect(swall, (vertices[i], vertices[(i+1) % 4])):
                    return True
            for dot in alien.get_head_and_tail():
                if point_segment_distance(dot, swall) <= w:
                    return True
    return False
...
def is_alien_within_window(alien: Alien, window: Tuple[int]):
    ...
    """
    vwindow =  ((0,window[1]),(0,0),(window[0],0),(window[0],window[1]))

    for swindow in ((vwindow[i], vwindow[(i+1)%4]) for i in range(4)):

        if alien.get_shape() == "Ball":
            loc, r = alien.get_centroid(), alien.get_width()
            if point_segment_distance(loc, swindow) <= r:
                return False
        elif alien.get_shape() == "Horizontal" or alien.get_shape() == "Vertical":
            x, y = alien.get_centroid()
            w, h = alien.get_width(), alien.get_length()/2
            ...
            for i in range(4):
                if do_segments_intersect(swindow, (vertices[i], vertices[(i+1) % 4])):
                    return False
            for dot in alien.get_head_and_tail():
                if point_segment_distance(dot, swindow) <= w:
                    return False
    return True
...
def is_point_in_polygon(point, polygon):
    ...
    """
        # all sign of dot product is the same
    out = []
    for i in range(4):
        a = np.array([polygon[i][0] - point[0], polygon[i][1] - point[1]])
        b = np.array([polygon[i][0] - polygon[(i+1)%4][0], polygon[i][1] - polygon[(i+1)%4][1]])
        out.append(np.cross(a,b))

    return (all((i > 0) for i in out)) or (all((i < 0) for i in out))
...
def does_alien_path_touch_wall(alien: Alien, walls: List[Tuple[int]], waypoint: Tuple[int, int]):
    ...
    for wall in walls:
        swall = ((wall[0], wall[1]), (wall[2], wall[3]))
        if alien.get_shape() == "Ball":
            if segment_distance((alien.get_centroid(), waypoint),swall) <= alien.get_width():
                return True
        elif alien.get_shape() == "Horizontal" or alien.get_shape() == "Vertical":

            x0, y0 = alien.get_centroid()
            x1, y1 = waypoint
            w, h = alien.get_width(), alien.get_length()/2
            ...
            # 0 -> before; 1 -> after
            if alien.get_shape() == "Horizontal":
                vertices0 = ((x0-h,y0-w), (x0-h,y0+w), (x0+h,y0+w), (x0+h,y0-w))
                vertices1 = ((x1-h,y1-w), (x1-h,y1+w), (x1+h,y1+w), (x1+h,y1-w))
            else:
                vertices0 = ((x0-w,y0-h), (x0-w,y0+h), (x0+w,y0+h), (x0+w,y0-h))
                vertices1 = ((x1-w,y1-h), (x1-w,y0+h), (x1+w,y0+h), (x1+w,y1-h))

            for i in range(4):
                # see if touching wall on the sweeping edges and original edges
                verticesCrt = (vertices0[i], vertices0[(i+1)%4], vertices1[(i+1)%4], vertices1[i])
                for j in range(4):
                    if do_segments_intersect((verticesCrt[j], verticesCrt[(j+1)%4]), swall):
                        return True
                # also see if wall inside polygon swept: 
                # if wall inside, above has sort out cases with intersect with edges,
                # therefore, just find out at least one node of wall inside the shape
                for node in swall:
                    if is_point_in_polygon(node, verticesCrt):
                        return True

            # see if circle on the head/tail touch wall
            for dot in alien.get_head_and_tail():
                if point_segment_distance(dot, swall) <= w:
                    return True
    return False
...
def point_segment_distance(p, s):
    ...
    """
    b, a = np.array([s[0][0] - p[0], s[0][1] - p[1]]), np.array([s[0][0] - s[1][0], s[0][1] - s[1][1]])
    # get project point h on oa
    h = a * max(0, min(1, np.dot(a,b)/np.dot(a,a)))
    #print(np.linalg.norm(b-h))
    return np.linalg.norm(b-h)
...
def do_segments_intersect(s1, s2):
    ...
    """
    p1, p2, q1, q2 =np.array(s1[0]), np.array(s1[1]), np.array(s2[0]), np.array(s2[1])
    p, q = p2 - p1, q2 -q1
    if ((np.cross(p, q1 - p1)* np.cross(p, q2 - p1) < 0 and np.cross(q, p1 - q1)* np.cross(q, p2 - q1) < 0) or 
        point_segment_distance(p1,s2) < 10e-5 or 
        point_segment_distance(p2,s2) < 10e-5 or
        point_segment_distance(q1,s1) < 10e-5 or 
        point_segment_distance(q2,s1) < 10e-5 ):
        return True
...
def segment_distance(s1, s2):
    ...
    if do_segments_intersect(s1,s2): 
        return 0
    #elif np.cross((np.array(s1[0])-np.array(s1[1])), (np.array(s2[0])-np.array(s2[1]))) == 0: 
    #    return -1
    else:
