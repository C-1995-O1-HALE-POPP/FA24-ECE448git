...
class EightPuzzleState(AbstractState):
    ...
    def swap_elements(self,zero_loc,new_zero_loc):
        ...
        temp = new_state[zero_loc[0]][zero_loc[1]]
        new_state[zero_loc[0]][zero_loc[1]] = new_state[new_zero_loc[0]][new_zero_loc[1]]
        new_state[new_zero_loc[0]][new_zero_loc[1]] = temp
        return new_state
    ...
    def compute_heuristic(self):
        ...
        goal_index, state_index = {}, {}
        for i in range(3):
            for j in range(3):
                goal_index[self.goal[i][j]] = (i,j)
                state_index[self.state[i][j]] = (i,j)
        for i in range(1,9):
            total += manhattan(goal_index[i],state_index[i])
